...
Modification History:
    ...
    - 2026-05-14 (김지우) : 로그인 및 회원가입 버튼을 AuthModalContext 기반 모달 호출로 변경
 */
...
export const LandingNav: React.FC<LandingNavProps> = ({ isAuthenticated, user, logout }) => {
  ...
  return (
    <nav className="fixed top-0 w-full z-[100] border-b border-zinc-200 bg-white">
      <div className="flex justify-between items-center w-full px-4 py-3 md:px-8 max-w-screen-2xl mx-auto">
        <div className="flex items-center gap-6 md:gap-10 min-w-0">
          <div className="flex items-center gap-2.5 cursor-pointer group min-w-0" onClick={handleLogoClick}>
            ...
            />
            <span className="text-xl font-bold tracking-tighter text-zinc-900 group-hover:text-[#0071e3] transition-colors pr-2 md:pr-4 whitespace-nowrap">Cadence AI</span>
          </div>
        </div>
