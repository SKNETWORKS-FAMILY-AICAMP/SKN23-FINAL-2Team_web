...
def get_all_organizations(
    ...
    return query.order_by(models.Organization.created_at.desc()).all()
...
def get_all_devices(org_id: Optional[str] = None, db: Session = Depends(get_db), current_admin: models.SystemAdmin = Depends(get_current_admin)):
    ...
    # 라이선스 정보를 거쳐야 하므로 조인 쿼리 사용 권장 (여기서는 간단하게 필터링)
    ...
    for dev in devices:
        ...
        result.append({
            ...
            "hostname": dev.hostname or "CAD-PC",
            "os_user": dev.os_user,
