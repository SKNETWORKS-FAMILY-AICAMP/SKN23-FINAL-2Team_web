      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="bg-gradient-to-br from-[#0071e3]/20 to-zinc-900 border border-[#0071e3]/30 p-8 rounded-3xl relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-6">
            ...
            <div className="pt-4 border-t border-white/10 text-sm flex justify-between">
              <span>결제 수단</span>
              <span>{paymentInfo?.pg_provider || '등록된 수단 없음'}</span>
            </div>
          </div>
        </div>
        <div className="bg-zinc-900/50 border border-white/10 p-8 rounded-3xl flex flex-col justify-center">
          <h4 className="font-bold text-white mb-2">구독 해지 안내</h4>
          <p className="text-sm text-zinc-400 mb-6">구독을 해지하시면 이번 결제 주기 종료 후 서비스 이용이 중단됩니다.</p>
          <button
            disabled={!isVerified}
            onClick={() => triggerConfirm('구독 해지', '정말 구독을 해지하시겠습니까?', () => toast.success('해지 예약되었습니다.'), 'red')}
            className={`w-full py-3 font-bold rounded-xl transition-all ${isVerified ? 'bg-red-500/10 text-red-500 hover:bg-red-500/20' : 'bg-white/5 text-zinc-600 cursor-not-allowed'
              }`}
          >
            {isVerified ? '구독 해지하기' : '승인 대기 중'}
          </button>
        </div>
      </div>
